...
Ingest’s landing page and all eight supporting pages share the bright neutral/amber theme in `site/ingest/assets/site.css`; landing-specific layout stays in `landing.css`. Current captures are listed under `images` in the screenshot manifest. The September 26 refresh uses 74 CC0 camera RAW samples for the main browsing, import, metadata, and preview examples; `raw-samples.json` records their sources and hashes. Download or verify the collection outside the repo with `python3 scripts/fetch_ingest_raw_samples.py /path/to/Public-RAW`. Superseded captures remain at their original URLs under `archivedImages`, with encoding and dimensions still verified.
