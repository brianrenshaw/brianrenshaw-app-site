...
Ingest is at `/ingest/`, the second Mac app. Its native source is in the private sibling `photo-importer` project. Public DMGs are GitHub Release assets in this repository, tagged `ingest-v*`; they are not committed into `site/`. See `INGEST_RELEASES.md` for the release handoff. The current download is signed and notarized Ingest 0.9.4.2, build 36.
...
Ingest’s landing page and all eight supporting pages share the bright neutral/amber theme in `site/ingest/assets/site.css`; landing-specific layout stays in `landing.css`. Current captures are listed under `images` in the screenshot manifest. The September 26 dark-mode refresh uses 155 licensed Signature Edits RAW samples: 26 classic wedding photographs, 40 landscapes, 39 personal photographs, 25 professional portraits of men and women, and 25 event photographs. The Wedding → Landscapes → Personal → Headshots → Events hero advances every six seconds with accessible manual controls and reduced-motion support. `raw-samples.json` records original names, sources, licenses, hashes, and demo filenames. Download or verify the collection outside the repo with `python3 scripts/fetch_ingest_raw_samples.py /path/to/Public-RAW`. Superseded captures remain at their original URLs under `archivedImages`, with encoding and dimensions still verified.

The homepage setup tabs show five genuine Settings captures and matching folder/naming/second-copy examples; all remain readable without JavaScript. Metadata Assist uses a 31-second native silent MP4/WebM recording with a lossless poster, text walkthrough, visibility-aware playback, persistent manual pause, and reduced-motion opt-in. `screenshots.json` also records video hashes and poster dimensions. Run `node scripts/check_ingest_interactions.cjs` for focused playback lifecycle checks, alongside the site and asset validators. See `SOURCES.md` for capture provenance and the distinction between disposable demo volumes and real backup drives.
