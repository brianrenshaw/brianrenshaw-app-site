...
Ingest’s landing page and all eight supporting pages share the bright neutral/amber theme in `site/ingest/assets/site.css`; landing-specific layout stays in `landing.css`. Current captures are listed under `images` in the screenshot manifest. The September 26 dark-mode refresh uses 154 licensed Signature Edits RAW samples: 26 classic wedding photographs, 40 landscapes, 39 personal photographs, 25 professional portraits of men and women, and 24 event photographs. The Wedding → Landscapes → Personal → Headshots → Events hero advances every six seconds with accessible manual controls and reduced-motion support. `raw-samples.json` records original names, sources, licenses, hashes, and demo filenames. Download or verify the collection outside the repo with `python3 scripts/fetch_ingest_raw_samples.py /path/to/Public-RAW`. Superseded captures remain at their original URLs under `archivedImages`, with encoding and dimensions still verified.
