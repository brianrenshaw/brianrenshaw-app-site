...
Photo Work and Photo Backup are two separately mounted disposable APFS disk images used to populate native destination controls. They share the host’s physical storage and are **not a resilient backup**. The public examples instruct readers to use separate physical drives and arrange off-site/catalog protection separately. Names and September 26 job dates are fictional organization examples; they do not alter original RAW capture dates. The isolated state is `/Users/Shared/Ingest Demo/Library`; photography is under `/Users/Shared/Ingest Demo/Photo Library`. No personal library or account appears.

`metadata-assist-demo-0.9.4.2.mp4` (H.264) and `.webm` (VP9) contain the same continuous 31-second silent recording of the actual released app at 2880 × 1800, 24 fps. Native `screencapture -v -V 60 -l` produced a 3104 × 2024 master. Encoding trims only the surrounding window shadow to the observed native bounds (`crop=2880:1800:112:76`) and ends the take at 31 seconds. There is no retiming, fake typing, compositing, instructional overlay, or reconstructed UI. The macOS recording indicator is part of the genuine captured window. The result remains visible for more than three seconds. The lossless poster `metadata-assist-result-dark-0.9.4.2-retina.webp` is a separate native 2x capture of that saved state.

The selected Yosemite sample is `Landscapes/Mountain Light/002-Mountain-Light.CR2`, originally `signature edit free raws - IMG_2031.CR2`, Canon EOS 6D. SHA-256: `660485046224b0cad01d3645eb783c8e0ec5c7cc4228358055a293fa02f8489a`. Its exact download URL and Signature Edits commercial-use license are in `raw-samples.json`. All 155 source RAW hashes were reverified after recording and match the manifest. Only its disposable XMP sidecar changed: the starting caption was Yosemite; Metadata Assist added IPTC Place Shown Yosemite Valley and keywords landscape, waterfall, Yosemite, and replaced the caption with Waterfall in Yosemite Valley. The semicolon picker labels Location as IPTC Place Shown. GPS was not edited. An empty malformed keyword element left by rehearsal was removed from the fixture before the final take; the final Apply succeeded and the actual inspector displayed all saved values.

Capture masters, sidecar evidence, and contact sheets were retained locally in `/tmp/ingest-interactive/` and `/tmp/ingest-polished/captures/`. Video hashes, codecs, duration, dimensions, and poster associations are recorded in `screenshots.json`. The older Metadata Assist and Workspaces stills remain active in the guide/getting-started pages; historical captures retain their existing archive entries and URLs.

The adaptable folder examples are informed by [Capture One Sessions](https://support.captureone.com/hc/en-us/articles/360003115137-The-Complete-Guide-to-Sessions-in-Capture-One). Backup copy explains the separate scope of [Adobe catalog backups](https://helpx.adobe.com/lightroom-classic/desktop/manage-catalogs-and-files/back-catalog.html) and [3-2-1 protection](https://www.backblaze.com/blog/the-3-2-1-backup-strategy/). MP4/WebM delivery follows [web.dev’s video guidance](https://web.dev/articles/replace-gifs-with-videos). Sources checked September 26, 2026.

# Current dark-mode demo library — September 26, 2026

All 19 active images in `screenshots.json` are genuine native Retina window captures from signed and notarized **Ingest 0.9.4.2 (36)** in dark appearance. The isolated demo library lives outside the home directory, under `/Users/Shared/Ingest Demo`; its sidebar root is **Photo Library**. No personal account or home-directory paths appear in these captures.
...
The collection contains 26 classic wedding photographs (white gowns, suits, veils, ceremony, reception, and detail photographs), 40 landscapes, 39 personal photographs, 25 professional portraits (men and women in ordinary clothing, with neutral poses), and 25 live music, reception, and community event photographs. These are separate photographs, not repeated filler or camera-test charts. Their original RAW formats and camera metadata were checked with ExifTool, including CFA checks for DNG files. Original capture dates are preserved; recent-folder order reflects demo organization. Wedding Selects, Mountain Light, Everyday Stories, Studio Portraits, and Event Highlights are neutral demonstration folders.

The five hero slides demonstrate Grid (Wedding), Split with histogram and camera information (Landscapes), Focus with filmstrip (Personal), Grid (Headshots), and Preview with two sharp face close-ups (Events). Headshots and Events have their own RAW sets and workspace filing presets. The original Landscapes and Personal hero captures are archived.

Capture command: `screencapture -x -o -a -l WINDOW_ID capture.png`. PNGs were exported as lossless WebP with decoded pixel equality verified. Main windows are 1440 × 900 logical pixels / 2880 × 1800 physical pixels; the Settings window is 1029 × 1290 / 2058 × 2580. No resizing, retouching, compositing, shadow, or cursor was added. A contact sheet and individual windows were reviewed for privacy, loading states, and framing. Metadata demonstrations use disposable copies and XMP sidecars; final fixture originals were restored where necessary and all 155 source checksums verified. Destination folders are disposable; no real client imports were performed.
...
### GPS Metadata Assist and workspace option captures — September 26, 2026

`metadata-assist-gps-0.9.4.2.mp4` and `.webm` replace the original place-name demonstration. Recorded directly from signed Ingest 0.9.4.2 (36) with `INGEST_TEST_LIBRARY=/Users/Shared/Ingest Demo/Library`. Continuous 34-second take, beginning with Metadata Assist open; only leading idle time and the native window shadow were trimmed. Native 2880 × 1592 pixels, silent, 24 fps, no speed changes, retouching, overlays, or reconstructed interface. The separate poster is a native lossless Retina window capture.

The selected RAW is `002-Mountain-Light.CR2`, originally `signature edit free raws - IMG_2031.CR2`, SHA-256 `660485046224b0cad01d3645eb783c8e0ec5c7cc4228358055a293fa02f8489a`. Original RAW and licensing provenance remain in `raw-samples.json`. The demonstration selects Yosemite Valley Welcome Center (37.746498, -119.584433) as an illustrative GPS example, not a claim about the photographer’s precise capture position. Caption, keywords, GPS, and place details were written to the demo XMP sidecar; original RAW camera metadata and bytes remain unchanged.

Five `workspace-*-options-dark-0.9.4.2-retina.webp` files are genuine 2058 × 2580 native Settings captures, displayed at no more than 1029 × 1290. Wedding: Grid, capture time, bursts, paired RAW+JPEG. Landscapes: Split, scene grouping, camera filenames. Personal: Organize hidden, Apple Photos enabled per workspace. Headshots: photo pairs and Skip video. Events: enabled `Event-caption-codes.tsv` and reload-on-change, with fictional event/venue/team codes in the isolated demo folder. Settings captures are lossless with decoded pixels verified against their native PNG sources. No personal account or home path is visible.

The previous video encodings and poster remain archived in `screenshots.json` without active page references. Workspace destination captures remain available in the expandable folder/backup examples. Native source verification: PresetEditor, MetadataTemplateEditor, and ScratchPad confirm workspace options, optional IPTC fields, local `.ingesttemplate` import/export, and TSV caption-code reload. TSV code lookup and XMP photograph metadata are distinct; the website does not promise automatic cloud synchronization.
